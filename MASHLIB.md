...
Produces `mashlib.js` (~220 KB minified, ~57 KB gzipped). One esbuild invocation, ~15 ms. The build script (`package.json`):
...
Public URL (after merge to `gh-pages`): `https://solid-apps.github.io/hub/mashlib.js`.
...
jss start --port 5446 --root ./pod-data --idp --notifications \
  --mashlib-module http://localhost:8004/mashlib.js
```
